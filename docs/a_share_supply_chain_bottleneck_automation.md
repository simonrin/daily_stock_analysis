...
可选，在 Settings -> Secrets and variables -> Actions -> Variables 配置：
...
如果只想测试生成报告、不发邮件，可把 ASTOCKANA_DISABLE_EMAIL=true。

## 输出格式约束

候选表固定为 7 列：

产业链层级/卡点环节 | 公司/股票代码 | 排序原因 | 一周新增证据 | 行情区间 | 主要风险 | 待验证事实/研究优先级

正文不显示收件人行、内部邮件标题行，也不显示“完整可筛选版本见附件...”之类附件说明句。表格直接出现在“核心候选公司横向对比表”标题后。

行情区间使用 Tushare，但表格中不显示数据源字样，不显示年份，不显示涨跌幅，只展示最近交易日收盘价、成交额和近 60 交易日历史价格区间。

## 手动触发

进入 Actions -> A-Share Supply Chain Bottleneck Weekly -> Run workflow。
