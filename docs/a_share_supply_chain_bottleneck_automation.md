...
| 产业链层级/卡点环节 | 公司/股票代码 | 排序原因 | 一周新增证据 | 行情/估值 | 主要风险 | 待验证事实/研究优先级 |
|---|---|---|---|---|---|---|
...
行情和估值使用 Tushare，但表格中不显示数据源字样，不显示年份、交易日期、成交额或近 60 交易日区间。E 列展示最近交易日收盘价、月涨跌幅、PEG、DCF 内在价值估算和行业均值对比。

DCF 采用统一保守参数：最近一期经营现金流减资本开支作为自由现金流基础，未来 5 年增长率参考净利润同比但封顶 25%、保底 2%，折现率 10%，永续增长率 2.5%。行业均值使用同表候选中 Tushare 行业分类相同样本的 `DCF 较现价` 均值；样本或数据不足时显示待复核。DCF 仅用于研究比较，不构成买卖建议。
